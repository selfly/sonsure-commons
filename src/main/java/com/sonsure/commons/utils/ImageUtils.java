...
public final class ImageUtils {
     ...
     */
    public static void resizeAddLogoSave(byte[] srcImage, byte[] waterImage, int maxImgWidth, float alpha,
                                         int pointX, int pointY, Integer degree, String targetPath) {
        try {
            BufferedImage srcImg = ImageIO.read(new ByteArrayInputStream(srcImage));
            ...
            ImageIcon imgIcon = new ImageIcon(ImageIO.read(new ByteArrayInputStream(waterImage)));
            ...
            throw new SonsureException("给图片添加水印出现错误", e);
        }
    }
    ...
    public static int[] getImageWH(byte[] bytes) {
        ...
        try {
            ...
            throw new SonsureException("读取图片长宽出现异常", e);
        }
    }
    ...
    public static byte[] cutImage(File imageFile, int x, int y, int cutWidth, int cutHeight) {
        try {
            ...
            throw new SonsureException("读取图片失败", e);
        }
    }
    ...
    private static byte[] bufferedImageToByte(BufferedImage bufferedImage, String imageFormat) {
        try (ByteArrayOutputStream bos = new ByteArrayOutputStream()) {
            ...
            throw new SonsureException("将BufferedImage转换成byte数组失败", e);
        }
    }
    ...
    private static byte[] readFileToByte(File srcFile) {
        try {
            ...
            throw new SonsureException("将图片文件转换成byte数组失败", e);
        }
    }
    ...
    private static BufferedImage readByteToBufferedImage(byte[] bytes) {
        ...
        try {
            ...
            throw new SonsureException("将byte数组转换成BufferedImage对象失败", e);
        }
    }
    ...
    private static void writeImage(BufferedImage bufferedImage, String formatName, File targetFile) {
        try {
            ...
            throw new SonsureException("图片写入失败", e);
        }
    }
    ...
    public static String getImageFormat(byte[] image) {
        ...
        try {
            ...
            throw new SonsureException("获取图片格式失败", e);
        } finally {
            try {
                iis.close();
            } catch (IOException e) {
